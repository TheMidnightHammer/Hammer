...
void HammerEngine::recreateSwapChain() {
...
void HammerEngine::populateDebugMessengerCreateInfo(VkDebugUtilsMessengerCreateInfoEXT& createInfo) {
    createInfo = {};
    createInfo.sType = VK_STRUCTURE_TYPE_DEBUG_UTILS_MESSENGER_CREATE_INFO_EXT;
    createInfo.messageSeverity = VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT;
    createInfo.messageType = VK_DEBUG_UTILS_MESSAGE_TYPE_GENERAL_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_TYPE_VALIDATION_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_TYPE_PERFORMANCE_BIT_EXT;
    createInfo.pfnUserCallback = HammerEngine::debugCallback;
}

void HammerEngine::setupDebugMessenger() {
...
void HammerEngine::createSurface() {
    if (glfwCreateWindowSurface(instance, window, nullptr, &surface) != VK_SUCCESS) {
        throw std::runtime_error("failed to create window surface!");
    }
}

void HammerEngine::pickPhysicalDevice() {
...
void HammerEngine::createLogicalDevice() {
    QueueFamilyIndices indices = findQueueFamilies(physicalDevice);
...
void HammerEngine::createSwapChain() {
...
void HammerEngine::createImageViews() {
...
void HammerEngine::createRenderPass() {
...
void HammerEngine::createCommandPool() {
...
void HammerEngine::createDepthResources() {
...
VkFormat HammerEngine::findSupportedFormat(const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features) {
...
VkFormat HammerEngine::findDepthFormat() {
...
bool HammerEngine::hasStencilComponent(VkFormat format) {
    return format == VK_FORMAT_D32_SFLOAT_S8_UINT || format == VK_FORMAT_D24_UNORM_S8_UINT;
}

VkImageView HammerEngine::createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, uint32_t mipLevels) {
...
void HammerEngine::generateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels) {
    // 1. Check if the image format supports linear blitting as source and destination
    ...
    for (uint32_t i = 1; i < mipLevels; i++) {
        // Transition mip level (i - 1) from TRANSFER_DST to TRANSFER_SRC
        ...
        // Record the blit command from (i - 1) to (i)
        VkImageBlit blit{};
    ...
    // Transition the final mip level (mipLevels - 1) from TRANSFER_DST to SHADER_READ_ONLY
...
void HammerEngine::createImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory) {
...
void HammerEngine::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, uint32_t mipLevels) {
    VkCommandBuffer commandBuffer = beginSingleTimeCommands();
...
void HammerEngine::createUniformBuffers() {
...
void HammerEngine::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory) {
...
VkCommandBuffer HammerEngine::beginSingleTimeCommands() {
...
void HammerEngine::endSingleTimeCommands(VkCommandBuffer commandBuffer) {
...
void HammerEngine::createCommandBuffers() {
...
void HammerEngine::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
    VkCommandBufferBeginInfo beginInfo{};
    beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
...
void HammerEngine::updateUniformBuffer(uint32_t currentImage) {
...
VkSurfaceFormatKHR HammerEngine::chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats) {
...
VkPresentModeKHR HammerEngine::chooseSwapPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes) {
...
SwapChainSupportDetails HammerEngine::querySwapChainSupport(VkPhysicalDevice device) {
...
bool HammerEngine::isDeviceSuitable(VkPhysicalDevice device) {
...
bool HammerEngine::checkDeviceExtensionSupport(VkPhysicalDevice device) {
...
QueueFamilyIndices HammerEngine::findQueueFamilies(VkPhysicalDevice device) {
...
std::vector<const char*> HammerEngine::getRequiredExtensions() {
...
bool HammerEngine::checkValidationLayerSupport() {
    uint32_t layerCount;
...
VKAPI_ATTR VkBool32 VKAPI_CALL HammerEngine::debugCallback(VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity, VkDebugUtilsMessageTypeFlagsEXT messageType, const VkDebugUtilsMessengerCallbackDataEXT* pCallbackData, void* pUserData) {
